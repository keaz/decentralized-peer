...
pub async fn client_connection(addr: impl ToSocketAddrs,mut broker_sender: Sender<Message>) -> Result<()> {
    ...
    let connect_command = Command::Connect { id: Uuid::new_v4(), client_id: peer_id.clone(), port: 123 };
    ...
    broker_sender.send(Message::NewPeer {id: Uuid::new_v4(),peer_id: peer_id.clone(),address: String::from("123") ,port: 0,stream: Arc::clone(&stream),})
        .await
        .unwrap();
